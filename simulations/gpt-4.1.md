Certainly! Let’s break down the approach and then provide R code.

## Approach

1. **Feature Engineering**:

    Useful features to predict arrivals (for each week):
    
    - Lag values (e.g., visa issued/applications from previous weeks), since arrivals likely lag behind visas issued.
    - Conversion ratios (e.g., visas issued / applications).
    - Visa arrival rate (arrivals / visas issued, by type).
    - Rolling averages or trends.
    - Separate by visa type.
    
2. **Model Choice**:
    - Given limited data, start with a simple model, like a weighted average/linear regression per visa type.
    - Use recent "visas issued" and typical lags between issue and arrival (compute from data) to project future arrivals.
   
3. **Simulation**:
    - Predict weekly arrivals for the next 3 months (assume 12 weeks).

---

## Sample R Implementation

Assume the data is a `data.frame` called `df`:
...
- **Assumptions**: This simulation assumes new visas issued weekly will continue at the most recent observed rate, and that arrival timing lags remain consistent.
- **Model Limits**: Simple average lags and rates are used; can improve by fitting a regression or time series model to arrival versus past issuances, by type.
- **Improving Model**: More features or using more advanced regression/time series models can improve results, but is data-dependent.

Adjust function and feature calculations based on your actual data column names and structure! If you provide a snippet of your actual data, I can further tailor the code for you.