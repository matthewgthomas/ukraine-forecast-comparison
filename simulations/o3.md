Below is a fully-worked template (with extensive comments) that shows one way to build a numerical simulation in R for forecasting weekly arrivals of people from Ukraine for the next three months (≈ 12 weeks).  
The code assumes you have a tidy data frame called df that contains:

• week            – Date (or character) giving the Monday (or other agreed start) of each week  
• visa_type       – Factor with levels "Ukraine Family Scheme", "Ukraine Sponsorship Scheme", "Government sponsored"  
• visa_apps       – Integer, total visa applications received in that week for that visa_type  
• visas_issued    – Integer, visas issued in that week for that visa_type  
• arrivals        – Integer, people with visas who actually arrived that week under that visa_type  

If your column names differ, adjust the code accordingly.
...
1. Separate model per visa scheme to capture the different processing / travel behaviour.  
2. Predict arrivals using lagged visas-issued, lagged arrivals and backlog (issued minus arrived) – all of which are known or can be asserted for the future.  
3. Draw 1000 bootstrap paths from the model residuals to generate a predictive distribution, not just a single point forecast.  
4. Aggregate the simulated arrivals across schemes to get the national weekly total and its uncertainty bands.

Improving it further
• Replace the naïve “median visas issued” assumption with its own time-series model or scenario planning (e.g., stepped increases when new sponsorship programmes open).  
• Include visa applications as a leading indicator of visas issued (two-stage model).  
• Allow seasonality or holiday effects by adding Fourier terms or week-of-year dummies.  
• If you want full Bayesian inference with richer uncertainty, swap the TSLM() NB model for a brms or rstanarm model and forward-simulate directly.

Remember: this is only as good as the input assumptions. Always communicate the intervals, not just the mean, and update the simulation as fresh weekly data arrive.